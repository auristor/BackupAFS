#============================================================= -*-perl-*-
#
# BackupAFS::CGI::RestoreInfo package
#
# DESCRIPTION
#
...
package BackupAFS::CGI::RestoreInfo;
...
sub action
{
    ...
    if ( $i >= @Restores ) {
        ErrorExit(eval("qq{$Lang->{Restore_number__num_for_volset__does_not_exist}}"));
    }

    %RestoreReq = ();
    do "$TopDir/volsets/$volset/RestoreInfo.$Restores[$i]{num}"
	    if ( -f "$TopDir/volsets/$volset/RestoreInfo.$Restores[$i]{num}" );
	...
	$targetFile =~ s{//+}{/}g;
        ...
        $f = decode_utf8($f);
	$fileListStr .= <<EOF;
<tr><td>$RestoreReq{volsetSrc}:/$strippedShareSrc$f</td><td>$RestoreReq{volsetDest}:/$strippedShareDest$targetFile</td></tr>
EOF
    }
    $RestoreReq{shareSrc}  = decode_utf8($RestoreReq{shareSrc});
    $RestoreReq{shareDest} = decode_utf8($RestoreReq{shareDest});
    my $content = eval("qq{$Lang->{Restore___num_details_for__volset2}}");
    Header(eval("qq{$Lang->{Restore___num_details_for__volset}}"),$content);
    Trailer();
}
