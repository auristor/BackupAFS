...
sub action
{
    ...
    my $backupAge = sprintf("%.1f", (time - $Backups[$i]{startTime})
                                    / (24 * 3600));
    my $view = BackupAFS::View->new($bafs, $volset, \@Backups, {nlink => 1});
	...
	foreach my $f ( sort {uc($a) cmp uc($b)} keys(%$attr) ) {
		...
		push(@DirStr, {needTick => 1,
			       ...
			       link     => <<EOF});
<a href="$MyURL?action=browse&volset=${EscURI($volset)}&num=$num&share=$shareURI&dir=$path">$imgStr</a><a href="$MyURL?action=browse&volset=${EscURI($volset)}&num=$num&share=$shareURI&dir=$path" class="$linkStyle">&nbsp;$dirName</a></td></tr>
EOF
                $fileCnt++;
                $gotDir = 1;
		if ( $dirOpen ) {
            ...
            if ( $relDir eq $dir ) {
                ...
                my ($attrStr, $iconStr);
                if ( defined($a = $attr->{$f}) ) {
                    my $mtimeStr = $bafs->timeStamp($a->{mtime});
		    # UGH -> fix this
                    my $typeStr  = BackupAFS::Attrib::fileType2Text(undef,
								   $a->{type});
		    my $statPath=$path;
...
EOF
    ...
    <td align="right" class="fviewborder">$sizeStr</td>
...
<tr><td class="fviewborder">
    <input type="checkbox" name="fcb$checkBoxCnt" value="$path">&nbsp;$iconStr&nbsp;<a href="$MyURL?action=browse&volset=${EscURI($volset)}&num=$num&share=$shareURI&dir=$path">$fDisp</a>
</td>
$attrStr
</tr>
EOF
                    ...
                    $fileStr .= <<EOF;
<tr><td class="fviewborder">
    <input type="checkbox" name="fcb$checkBoxCnt" value="$path">&nbsp;$iconStr&nbsp;<a href="$MyURL?action=RestoreFile&volset=${EscURI($volset)}&num=$num&share=$shareURI&dir=$path">$fDisp</a>
</td>
$attrStr
</tr>
EOF
    ...
    if ( my @otherDirs = $view->backupList($share, $dir) ) {
	    ...
	    my $backupNum = $Backups[$i]{num};
            ...
            $otherDirs .= "<option value=\"$MyURL?action=browse&volset=${EscURI($volset)}&num=$backupNum&share=$shareURI&dir=$pathURI\"$selected>#$backupNum - ($showDate)</option>\n";
        }
