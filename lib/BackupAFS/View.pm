#============================================================= -*-perl-*-
#
# BackupAFS::View package
#
# DESCRIPTION
#
#   This library defines a BackupAFS::View class for merging of
...
sub new
{
    my($class, $bafs, $volset, $backups, $options) = @_;
    my $m = bless {
        bafs       => $bafs,	# BackupAFS::Lib object
        volset      => $volset,	# volset name
        backups   => $backups,	# all backups for this volset
	num       => -1,	# backup number
        ...
        dirOpts   => $options,  # $options is a hash of file attributes we need:
                                ...
                                # See BackupAFS::Lib::dirRead().
    }, $class;
    $m->{topDir} = $m->{bafs}->TopDir();
    return $m;
}
...
sub dirCache
{
	...
	my $path     = "$m->{topDir}/volsets/$m->{volset}/$backupNum/";
	if ( -d "$path/f%2f" ) { $mangle = 1; } else { $mangle = 0; }
	my $legacyCharset = $m->{backups}[$i]{version} < 3.0;
        my $sharePathM;
        if ( $mangle ) {
            $sharePathM = $m->{bafs}->fileNameEltMangle($share)
                        . $m->{bafs}->fileNameMangle($dir);
        } else {
        ...
        if ( $legacyCharset ) {
            $dirOpts->{charsetLegacy}
                    ...
                    = $m->{bafs}->{Conf}{ClientCharsetLegacy} || "iso-8859-1";
        }

        my $dirInfo = $m->{bafs}->dirRead($path, $dirOpts);
	if ( !defined($dirInfo) ) {
            if ( $i == $m->{idx} ) {
                #
        ...
        my $attr;
	if ( $mangle ) {
	    $attr = BackupAFS::Attrib->new($attribOpts);
	    if ( !$attr->read($path) ) {
        ...
        foreach my $entry ( @$dirInfo ) {
            ...
            my $fileUM = $file;
            $fileUM = $m->{bafs}->fileNameUnmangle($fileUM) if ( $mangle );
		...
		$attr->set($fileUM, undef);
                ...
                if ( $compress && -f _ ) {
                    ...
                    #
                    my $f = BackupAFS::FileZIO->open($realPath,
						    0, $compress);
                    if ( !defined($f) ) {
	...
	if ( defined($attr) ) {
		...
		my $file = $fileUM;
		$file = $m->{bafs}->fileNameMangle($fileUM) if ( $mangle );
