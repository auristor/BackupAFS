...
You can start a restore that will restore these volume(s) directly into
AFS.
</p><p>
<b>Warning:</b> If you do not specify an extension, the VOLUME YOU'RE
RESTORING WILL BE OVERWRITTEN!
</p>
<form action="\$MyURL" method="post" name="direct">
<input type="hidden" name="host" value="\${EscHTML(\$host)}">
<input type="hidden" name="num" value="\$num">
...
<table border="0">
<tr>
    <td>Restore the files to host (don't change)</td>
    <td><!--<input type="text" size="40" value="\${EscHTML(\$host)}"
	 name="hostDest">-->
	 <select name="hostDest" onChange="document.direct.shareDest.value=''">
	 \$hostDestSel
    ...
    <td>Pass the following options to \"vos restore\"</td>
    <td><input type="text" size="40" value="\${EscHTML(\$share)}"
	 name="shareDest"></td>
</tr><tr>
    <td>Unused for AFS restores (don't change)</td>
    <td valign="top"><input type="text" size="40" maxlength="256"
	value="\${EscHTML(\$pathHdr)}" name="pathHdr"></td>
</tr><tr>
    <td><input type="submit" value="Start Restore" name="ignore"></td>
