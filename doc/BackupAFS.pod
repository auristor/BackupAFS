...
Any standard linux or unix file system should work fine. NFS mounted file systems work too. Windows based FAT and NTFS file systems have not been tested, but should work in principle (BackupAFS does not use hard links, but depending some paths may get very long, depending on both site data store location and volume- and volumeset-naming decisions.
...
located in the following locations:
    ...
    /etc/BackupAFS/pc/HOST.pl   per-pc config file
...
The full list of fields which represent volume entries is
    ...
    Entry1_Servers, Entry1_Partitions, Entry1_Partitions
    Entry2_Servers, Entry2_Partitions, Entry2_Partitions
    Entry3_Servers, Entry3_Partitions, Entry3_Partitions
    Entry4_Servers, Entry4_Partitions, Entry4_Partitions
    Entry5_Servers, Entry5_Partitions, Entry5_Partitions 
...
Please see the Wiki at L<http://backupafs.wiki.sourceforge.net> for
...
Additionally, the default backup directory (datadir) in BackupPC4AFS in $Conf{TopDir}/pc. For example, /srv/BackupPC/pc. BackupAFS stores its volume backups in $Conf{TopDir}/volsets. For example, __TOPDIR__/volsets.
...
B<This program (BackupAFS) is released under AGPL v3 license (no other versions).>
