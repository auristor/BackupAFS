...
# AFS additions
# document these
# Path to the voswrap.pl script on the server
$Conf{VosPath} = '$Installdir/bin/voswrap.pl';

# Full command to run voswrap.pl on the server for a backup.
$Conf{VosCmd} = '$voswrapPath \\
 --volume=$shareName --type=$type --incrDate=$incrDate \\
 --incrLevel=$incrLevel --clientDir=$topDir/pc/$client \\
 --dest=$topDir/pc/$client/new';

# Full command to run voswrap.pl on the server for a restore
$Conf{VosRestoreCmd} = '$voswrapPath --volume=$shareName \\
 --type=$type --clientDir=$topDir/pc/$client \\
 --restoreDir=$restoreDir --bkupSrcNum=$bkupSrcNum \\
 --bkupSrcHost=$bkupSrcHost --fileList=$fileList';
